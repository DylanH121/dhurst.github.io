...
function search() {
    ...
    let i;
    const keyword = document.querySelector("#txt-search").value;
    ...
    if (keyword === "") {
      ...
      for (i = 0; i < videosList.length; i++) {
          if (videosList[i].description.includes(keyword)) {
              searchResultIndexes[searchResultIndexes.length] = i;
          }
      }
  
      if (searchResultIndexes.length === 0) {
          document.querySelector("#results").innerHTML = `No videos found with the keyword ${keyword}`;
      }
      else {
          document.querySelector("#results").innerHTML = "";
  
          for (i = 0; i < searchResultIndexes.length; i++) {
              document.querySelector("#results").innerHTML += generateRowHTML(videosList[searchResultIndexes[i]]);
          }
          searchResultIndexes = [];
      }
    }
